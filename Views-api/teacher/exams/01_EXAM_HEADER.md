# 🎨 01. مواصفات ومكون هيدر صفحة الامتحانات للمعلم (`TeacherExamHeader`)

> **الملف المرجعي:** [`Views-api/teacher/exams/01_EXAM_HEADER.md`](file:///c:/Users/alhareth/Desktop/work/school-system/Views-api/teacher/exams/01_EXAM_HEADER.md)  
> **اسم المكون البرمجي:** `TeacherExamHeader.vue`  
> **التصميم المرجعي المعتمد:** مطابق 100% لمواصفات [`Views-api/student/exams/01_EXAM_HEADER.md`](file:///c:/Users/alhareth/Desktop/work/school-system/Views-api/student/exams/01_EXAM_HEADER.md)  
> **الوصف:** الهيدر العلوي بنمط iOS 18 مع الجزيرة التفاعلية (Dynamic Island)، وشريط العنوان وزر الرجوع، وبطاقة الهيرو البيضاء العائمة المتداخلة (`ios-hero-card`) مع مؤشر الحالة الحي وصورة الامتحانات ثلاثية الأبعاد (3D).
...
1. **شريط الحالة (iOS Status Bar):** وقت النظام (`9:41`)، الجزيرة التفاعلية (`Dynamic Island` مع عدسة الكاميرا `camera-lens`)، وأيقونات الإشارة والواي فاي والبطارية بنمط Apple الأصيل.
2. **شريط الملاحة (Nav Bar):**
   - **زر الرجوع (Back Button):** زر دائري زجاجي مع سهم للرجوع للوحة تحكم المعلم.
   - **عنوان الصفحة الرئيسي:** `"جدول الامتحانات"` بظلال متألقة.
   - **زر الفلترة/الإجراء السريع:** زر دائري زجاجي.
3. **بطاقة الهيرو العائمة المتداخلة (Floating Hero Card):**
   - كرت أبيض عائم بنتوء سلبي متداخل مع البانر (`margin: -55px 16px 0`).
   - **العنوان الرئيسي:** `"إدارة وجدولة الامتحانات والاختبارات"`.
   - **شارة المعلم والشعب:** كبسولة بنفسجية (`#ede9fe` مع نص `#5b21b6`).
   - **الفصل الدراسي:** `"الفصل الدراسي الأول 2026"`.
   - **مربع المجسم 3D:** صندوق بتدرج ناعم مع صورة الامتحانات 3D ونقطة الحالة الخضراء المضيئة (`status-indicator-dot`).

---

## 📐 2. كود المكون الكامل Vue 3 Component (`TeacherExamHeader.vue`)
  ...
  <header class="ios-header">
    ...
    <div class="ios-header-banner">
      ...
      <div class="exam-nav-bar">
        ...
        <button class="nav-icon-btn filter-btn" @click="$emit('open-filter')" title="تصفية الفصول">
    ...
    <div class="ios-hero-card animate-fade-in">
      <div class="hero-text-content">
        ...
        <div class="hero-class-badge">{{ teacherName }} • {{ assignedSectionsText }}</div>
        <span class="hero-term-text">{{ semesterInfo }}</span>
      </div>

      <div class="hero-illustration-wrapper">
        <div class="hero-3d-box">
          <img :src="heroImage || examImg || '/images/exams_evaluations_3d.png'" class="exams-3d-img" alt="امتحانات 3D" />
...
const props = defineProps({
  title: { type: String, default: 'جدول الامتحانات' },
  subtitle: { type: String, default: 'إدارة وجدولة الامتحانات والاختبارات' },
  teacherName: { type: String, default: 'أ. أحمد سالم' },
  subjectSpecialty: { type: String, default: 'معلم الرياضيات' },
  assignedSectionsText: { type: String, default: 'الصف 5 (أ5 ، ب5)' },
