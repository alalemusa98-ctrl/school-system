# 📑 02. مواصفات ومكون تابات وفلاتر المعلم (`TeacherHomeworkTabs`)

> **الملف المرجعي:** [`Views-api/teacher/homeworks/02_HOMEWORK_TABS.md`](file:///c:/Users/alhareth/Desktop/work/school-system/Views-api/teacher/homeworks/02_HOMEWORK_TABS.md)  
> **اسم المكون البرمجي:** `TeacherHomeworkTabs.vue`  
> **التصميم المرجعي المعتمد:** مطابق 100% لمواصفات [`Views-api/student/homeworks/02_HOMEWORK_TABS.md`](file:///c:/Users/alhareth/Desktop/work/school-system/Views-api/student/homeworks/02_HOMEWORK_TABS.md)  
> **الوصف:** المكون المخصص للتحكم المقسم (Segmented Control) للتنقل بين الواجبات الحالية والأرشيف مع عداد نوتيفيكيشن أحمر، وزر نشر الواجبات الفوري، وشريط كبسولات الفصول والشعب المسندة للمعلم.
...
+-----------------------------------------------------------------------+
|  +-----------------------------------------------------------------+  |
|  |  (📁) الأرشيف                    (🔴 4) (🕒) الواجبات الحالية   |  |
|  +-----------------------------------------------------------------+  |
|                                                                       |
|  +-----------------------------------------------------------------+  |
|  |             [ ➕ نشر واجب مدرسي جديد للشعبة ]                    |  |
|  +-----------------------------------------------------------------+  |
|                                                                       |
|  +--------+   +---------------+   +---------------+   +-----------+   |
|  |   🌟   |   |      🏫       |   |      🏫       |   |    🏫     |   |
...
1. **التحكم المقسم السريع (Segmented Control):**
   - **الواجبات الحالية:** زر أبيض عائم بظل متميز وأيقونة ساعة مع عداد دائري أحمر (`4`).
   - **الأرشيف:** زر شفاف بأيقونة مجلد.
2. **زر إضافة واجب جديد (CTA Action):**
   - زر بارز بتدرج بنفسجي/أرجواني زجاجي مع أيقونة إضافة `+` وشارة `"فوري"`.
3. **شريط كبسولات الشعب والفصول (Vertical Pills Row):**
   - كبسولات تفاعلية مصفوفة أفقياً وقابلة للسحب السلس مع أيقونة عمودية في الأعلى واسم الشعبة في الأسفل.
   - **كبسولة "كافة الشعب":** بنجمة ذهبية (`🌟`) وتتحول للتدرج البنفسجي المضيء عند التفعيل.

---

## 📐 2. كود المكون الكامل Vue 3 Component (`TeacherHomeworkTabs.vue`)
...
<template>
  <section class="hw-filter-section">
    ...
    <button class="hw-create-action-btn" @click="$emit('open-create')">
      ...
      <span class="create-btn-label">نشر واجب مدرسي جديد للشعبة</span>
      <span class="create-btn-tag">فوري</span>
    </button>
...
const props = defineProps({
  ...
  currentCount: { type: Number, default: 4 },
  archiveCount: { type: Number, default: 2 },
