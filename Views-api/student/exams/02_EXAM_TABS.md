# 📑 02. مواصفات ومكون تابات وفلاتر مواد الامتحانات (`ExamTabs`)

> **الملف المرجعي:** [`Views-api/student/exams/02_EXAM_TABS.md`](file:///c:/Users/alhareth/Desktop/work/school-system/Views-api/student/exams/02_EXAM_TABS.md)  
> **اسم المكون البرمجي:** `ExamTabs.vue`  
> **الوصف:** المكون المخصص للتابات السريعة وفلاتر التصفية حسب المادة لصفحة الامتحانات (الجزء الثاني في التصميم المعتمد)، ويتكون من زر التحكم المقسم (Segmented Control) للتنقل بين الامتحانات القادمة والنتائج/الأرشيف مع عداد نوتيفيكيشن أحمر، بالإضافة لصف كبسولات فلاتر المواد الدراسية.
...
1. **التحكم المقسم السريع (Segmented Control):**
   - **تاب الامتحانات القادمة (Upcoming Exams Tab):** زر كبسولي عائم بخلفية بيضاء وظل متميز يحتوي على أيقونة تقويم، نص `"الامتحانات القادمة"`، وعلامة نوتيفيكيشن حمراء دائرية تحتوي على عدد الامتحانات القادمة (`3`).
   - **تاب النتائج والأرشيف (Results Archive Tab):** زر كبسولي يحتوي على أيقونة نجمة ونص `"النتائج والأرشيف"`.
2. **شريط كبسولات المواد (Vertical Subject Pills Row):**
   ...
   - **كبسولة "الكل":** الكبسولة النشطة ملونة بالتدرج الوردية/الأحمر المضيء بنجمة ذهبية 3D (`🌟`).
...
## 📐 2. كود المكون الكامل Vue 3 Component (`ExamTabs.vue`)
...
## ⚙️ 3. الـ Props و الـ Events الخاصة بـ `ExamTabs`

| اسم الـ Prop / Event | النوع | الوصف | القيمة الافتراضية |
| :--- | :--- | :--- | :--- |
| `activeTab` | `String` | اسم التاب المفعّل (`'upcoming'` أو `'results'`) | `'upcoming'` |
| `selectedSubject` | `String` | كود المادة المحددة (`'ALL'`, `'math'`, الخ) | `'ALL'` |
| `upcomingCount` | `Number` | عدد الامتحانات القادمة لعرض شارة العداد | `3` |
