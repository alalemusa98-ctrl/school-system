# 📚 03. مواصفات ومكون عرض الواجبات (`HomeworkList`)

> **الملف المرجعي:** [`Views-api/student/homeworks/03_HOMEWORK_LIST.md`](file:///c:/Users/alhareth/Desktop/work/school-system/Views-api/student/homeworks/03_HOMEWORK_LIST.md)  
> **اسم المكون البرمجي:** `HomeworkList.vue`  
> **الوصف:** المكون الرئيسي المخصص لعرض الواجبات المدرسية وكروت الأيام (الجزء الثالث المحدد بالمستطيل البنفسجي في التصميم المعتمد)، حيث يحتوي على كروت المجموعات اليومية المزودة برقم النتوء العلوي (Notch Badge)، عنوان اليوم، شبكة بطاقات المواد الثنائية (Squircle Grid)، تاريخ التسليم في الأسفل، وتكامل الدرج الجاني (Side Drawer) والنافذة المنبثقة للحل النموذجي (Model Solution Modal).
...
### عناصر عرض الواجبات الكروت الأساسية:
1. **النتوء العلوي العائم (Top Notch Badge):** دائرة مرقمة متمركزة في النتوء العلوي للكارد لترتيب مجموعات الأيام (`1` في الصورة المرجعية).
2. **رأس الكارد اليومي (Header Title):** عنوان واصل يوضح اسم اليوم `"واجبات يوم الأحد"`.
3. **شبكة السكواركل المزدوجة (2-Column Squircle Grid):**
   - شبكة ثنائية لبطاقات المواد القابلة للنقر (`hw-squircle-click`).
   - **صندوق الأيقونة:** أيقونة المادة 3D أو الإيموجي داخل مربع دائر الحواف متناسق مع لون المادة.
   - **شارة الحالة:** كبسولة ملونة توضح حالة التسليم (`"معلق"`, `"تم الإرسال"`, أو الدرجة المكتسبة `"10 / 10"`).
...
5. **الدرج النافذة (Side Drawer & Modal):** تفاصيل الواجب والحل النموذجي المعتمد عند النقر على أي مادة.

---

## 📐 2. كود المكون الكامل Vue 3 Component (`HomeworkList.vue`)
...
<script setup>
import { ref } from 'vue';
import ShadcnDrawer from '../../common/ShadcnDrawer.vue';
...
## ⚙️ 3. الـ Props و الـ Events الخاصة بـ `HomeworkList`

| اسم الـ Prop / Event | النوع | الوصف | القيمة الافتراضية |
| :--- | :--- | :--- | :--- |
| `loading` | `Boolean` | حالة التحميل من الـ Backend | `false` |
| `activeTab` | `String` | التاب الحالي النشط (`'current'` أو `'previous'`) | `'current'` |
| `groupedCurrentHomeworks` | `Array` | مجموعات الواجبات الحالية مفرزة حسب التاريخ | `[]` |
| `groupedArchiveHomeworks` | `Array` | مجموعات الواجبات المكتملة في الأرشيف | `[]` |
